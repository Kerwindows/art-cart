...
export const AuthProvider = ({ children }) => {
  ...
  const loginHandler = async (e, email, password) => {
    ...
    try {
      ...
      setLoginLoading(false);
      console.error("Login Error:", error); // keep this for dev debugging

      const message =
        error?.response?.data?.errors?.[0] || // try the common format
        error?.response?.data?.message || // fallback if message field exists
        error?.message || // fallback to native JS error message
        "Login failed. Please try again."; // final default message
