...
export const CartAmountSummary = ({ couponSelected, onCheckout }) => {
  const { userDataState, dispatch } = useUserData();
  ...
  const placeOrderHandler = () => {
    ...
    onCheckout(); // Open the modal after setting order details
  };
  ...
  return (
    <div className="cart-price-container">
      ...
      <div className="discount-container">
        <span>Discount: </span>
        <span>-${(totalOriginalPrice - totalDiscountedPriceBeforeCoupon).toFixed(2)}</span>
      </div>
      {isCouponApplied && (
        <div className="discount-container">
          <span>Coupon Discount: </span>
